...
| `/handoff-ingest` | Закрыть распухший чат: записать передачу дел в `docs/HANDOFF.md`, пометить чат 🚫 |
| `/handoff-query` | Первая команда нового чата: принять передачу, сделать вики старой сессии, доложить состояние |

Механика каждой команды — в [SKILLS.md](SKILLS.md) (единый каталог, дополняется).
